		...
		for (Class temp : classes) {
			...
			String sql = "CREATE TABLE IF NOT EXISTS " + tableName + " ("
				+ primaryKey + " varchar(20) primary key";
					//	String sql="IF NOT EXISTS ( SELECT * FROM	dbo.SysObjects	WHERE "
			//		+" ID = object_id('"+tableName+"') ) CREATE TABLE "+tableName+" ("
			//	+ primaryKey + " varchar(20) primary key";
			
			Method[] methods = temp.getDeclaredMethods();
			Set<String> filedSqls = new HashSet<String>();
			
			filedSqls.add(primaryKey + " varchar(225) primary key");
